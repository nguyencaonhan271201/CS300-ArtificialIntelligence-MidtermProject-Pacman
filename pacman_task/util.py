...
class Stack:
    ...
    def put(self, val):
        #Assume the top of the stack is the end of the array
        #Process of putting an element into the stack is equivalent to appending into the list
        self.__stack__.append(val)

    def pop(self):
        #Return None if the queue is empty
        #else pop the element at the top and return the value
        return None if self.is_empty() else self.__stack__.pop()

    def size(self):
        return len(self.__stack__)

    def top(self):
        return None if self.is_empty() else self.__stack__[-1]

    def bottom(self):
        return None if self.is_empty() else self.__stack__[0]
...
class Queue:
    ...
    def dequeue(self):
        #Return None if the queue is empty
        #else dequeue the first element and return the value
        return None if self.is_empty() else self.__queue__.pop(0)
    ...
    def front(self):
        ...
        return None if self.is_empty() else self.__queue__[0]

    def back(self):
        #Return None if the queue is empty
        #else return the last element
        return None if self.is_empty() else self.__queue__[-1]
...
class PriorityQueue:
    ...
    def push(self, item, priority):
        hq.heappush(self.__pq__, (priority, self.__ID__, item))
        self.__ID__ += 1

    def pop(self):
        return None if self.isEmpty() else hq.heappop(self.__pq__)

    def isEmpty(self):
        return len(self.__pq__) == 0

    def update(self, item, priority):
        for i, (p, size, it) in enumerate(self.__pq__):
        # If item already in priority queue with higher priority, update its priority and rebuild the heap.
        # If item already in priority queue with equal or lower priority, do nothing.
            if it == item and priority < p:
                self.__pq__[i] = (priority, size, it)
                hq.heapify(self.__pq__)
                return
...
class PriorityQueueWithFunction(PriorityQueue):
    ...
    '''
    def __init__(self, function, problem=None):
        super().__init__()
